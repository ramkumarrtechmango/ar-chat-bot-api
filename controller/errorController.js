...
const handleMongoValidationError = (err) => {
  ...
  if (errors.length) {
    if (errors.length > 1) {
      ...
      return new AppError(formattedErrors, 400, fields);
    } else {
      return new AppError(errors[0], 400, fields);
    }
  }
  return new AppError('Something went wrong - please try again later', 400);
};
